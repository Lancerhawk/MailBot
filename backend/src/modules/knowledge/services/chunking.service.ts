...
function isHeading(line: string): boolean {
  ...
  if (/^\d+(\.\d+)*\.?\s+\S/.test(trimmed) && trimmed.length < 100) return true;

  if (trimmed.length > 3 && trimmed.length < 80 && trimmed === trimmed.toUpperCase() && /[A-Z]/.test(trimmed)) return true;
...
function chunkSection(section: Section, version: number, startIndex: number): ChunkResult[] {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (paraTokens > MAX_CHUNK_SIZE) {
      ...
      let sentenceBuffer: string[] = [...overlapSentences];
...
export class ChunkingService {
  chunkText(text: string, version: number): ChunkResult[] {
