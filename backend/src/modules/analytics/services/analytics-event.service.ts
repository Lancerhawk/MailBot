    ...
    try {
      (async () => {
        try {
          ...
          if (metrics) {
            if (metrics.promptTokens) updateData.totalPromptTokens = { increment: metrics.promptTokens };
            if (metrics.completionTokens) updateData.totalCompletionTokens = { increment: metrics.completionTokens };
            if (metrics.estimatedCost) updateData.estimatedCost = { increment: metrics.estimatedCost };
            if (metrics.timeSavedSeconds) updateData.timeSavedSeconds = { increment: metrics.timeSavedSeconds };
            if (metrics.storageUsedBytes) updateData.storageUsedBytes = { increment: metrics.storageUsedBytes };
          ...
          let activityAction: any = null;
          let entityType: string | undefined = undefined;

          switch (eventType) {
            case AnalyticsEventType.EMAIL_RECEIVED: activityAction = 'EMAIL_RECEIVED'; entityType = 'Email'; break;
            case AnalyticsEventType.EMAIL_ANALYZED: activityAction = 'EMAIL_ANALYZED'; entityType = 'Email'; break;
            case AnalyticsEventType.EMAIL_REPLIED: activityAction = 'EMAIL_SENT'; entityType = 'Email'; break;
            case AnalyticsEventType.DRAFT_APPROVED: activityAction = 'DRAFT_APPROVED'; entityType = 'Draft'; break;
            case AnalyticsEventType.DOCUMENT_EMBEDDED: activityAction = 'DOCUMENT_EMBEDDED'; entityType = 'Document'; break;
            case AnalyticsEventType.CONTACT_CREATED: activityAction = 'CONTACT_CREATED'; entityType = 'Contact'; break;
            case AnalyticsEventType.ORGANIZATION_CREATED: activityAction = 'ORGANIZATION_CREATED'; entityType = 'Organization'; break;
            case AnalyticsEventType.PROCESSING_FAILURE: activityAction = 'SYSTEM_ERROR'; entityType = 'System'; break;
          }
