...
export class GmailDbService {
  ...
  async listThreads(userId: string, page: number, limit: number, filter?: string, search?: string) {
    const skip = (page - 1) * limit;

    let emailCondition: any = { isDeleted: false, isDraft: false };
    ...
    const totalEmails = await prisma.email.count({ where: { userId, isDeleted: false, isSpam: false } });
  ...
  async getEmailByIdWithConnection(userId: string, emailId: string) {
  ...
  async upsertThreadAndEmails(
    ...
    const latestEmail = filteredEmails.reduce((latest, current) =>
    ...
    for (const parsed of filteredEmails) {
      ...
      if (existingEmail && existingEmail.processingStatus === 'SKIPPED') {
        if ((existingEmail.isSpam && !newIsSpam) || (existingEmail.isDeleted && !newIsDeleted)) {
          processingStatusToUpdate = ProcessingStatus.PENDING;
        }
      }
      ...
      const existingParticipants = await prisma.emailParticipant.count({ where: { emailId: email.id } });
      ...
      await prisma.emailLabel.deleteMany({ where: { emailId: email.id } });
      ...
      const existingAttachments = await prisma.attachment.count({ where: { emailId: email.id } });
