...
export class WatchRenewalService {
  ...
  private async performWatchRegistration(userId: string, attempt = 1, force = false): Promise<void> {
    try {
      ...
      if (force) {
        try {
          ...
          logger.info(`[WatchRenewal] Cleared old Gmail watch state for user ${userId}`);
        } catch (stopErr: any) {
          logger.warn(`[WatchRenewal] Could not stop existing watch (benign if none existed): ${stopErr?.message}`);
        }
      }
      ...
      if (isScopeError) {
        logger.warn(`[WatchRenewal] User ${userId} has insufficient Gmail OAuth scopes or expired token. Skipping retries until re-authentication.`);
        await prisma.emailAccountConnection.updateMany({
          where: { userId, provider: 'GMAIL' },
