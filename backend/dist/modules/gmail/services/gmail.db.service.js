...
class GmailDbService {
    ...
    async getThreadsBulk(userId, threadIds) {
        return prisma_1.prisma.emailThread.findMany({
    ...
    async getPendingEmailIds(emailIds) {
        if (!emailIds || emailIds.length === 0)
            return [];
        const pendingEmails = await prisma_1.prisma.email.findMany({
            where: {
                id: { in: emailIds },
