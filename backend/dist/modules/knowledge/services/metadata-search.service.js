...
exports.MetadataSearchService = void 0;
const prisma_1 = require("../../../lib/prisma");
const logger_1 = require("../../../config/logger");
class MetadataSearchService {
    async searchDocuments(userId, queryText, limit = parseInt(process.env.METADATA_TOP_K || '5', 10)) {
        ...
        try {
            ...
            if (!tsQueryParam)
                return [];
            const results = await prisma_1.prisma.$queryRawUnsafe(`WITH search_data AS (
          SELECT
            d.id,
            d.title,
            d.description,
        ...
        ORDER BY score DESC
        LIMIT $4`, userId, tsQueryParam, queryText, limit);
            ...
            logger_1.logger.warn({ err: err instanceof Error ? err.message : err, userId }, 'Primary metadata search failed (pg_trgm might be missing). Falling back to basic ILIKE search.');
            try {
                const fallbackRegex = cleanQuery.split(/\s+/).filter(w => w.length > 2).join('|');
                if (!fallbackRegex)
                    return [];
                const fallbackResults = await prisma_1.prisma.$queryRawUnsafe(`SELECT
            d.id,
            1.0 as score
          FROM "KnowledgeBaseDocument" d
          WHERE d."userId" = $1
          ...
          LIMIT $3`, userId, fallbackRegex, limit);
                ...
                logger_1.logger.error({ err: fallbackErr, userId }, 'Fallback metadata search failed');
                return [];
            }
        }
    }
}
exports.MetadataSearchService = MetadataSearchService;
