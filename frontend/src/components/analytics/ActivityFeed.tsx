...
export function ActivityFeed({ filterParams }: { filterParams?: Record<string, string> }) {
  ...
  return (
    <motion.div 
      ...
      className="bg-zinc-900/50 border border-zinc-800/80 rounded-xl overflow-hidden flex flex-col h-full"
    >
      <div className="p-5 border-b border-zinc-800/80 bg-zinc-900/80 shrink-0 z-20 relative">
        <h3 className="text-lg font-medium text-zinc-200">All Activities</h3>
      </div>
          ...
          <Tooltip.Provider delayDuration={100}>
                  ...
                  >
                    <Tooltip.Root>
                      <Tooltip.Trigger asChild>
                        <div className="flex-1 min-w-0 p-4 rounded-xl border border-zinc-800/60 bg-zinc-900/40 shadow-sm hover:bg-zinc-800/80 transition-colors cursor-default text-left block w-full">
                          <div className="flex items-start gap-3">
                            <div className="flex items-center justify-center w-8 h-8 rounded-full border border-zinc-800/80 bg-zinc-900 shadow-sm shrink-0 transition-colors group-hover:bg-zinc-800 group-hover:border-zinc-700 mt-0.5">
                              {getIcon(activity.action)}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="font-semibold text-zinc-200 text-sm tracking-wide mb-0.5 leading-tight">
                                {formattedAction}
                              </div>
                      ...
                      <Tooltip.Portal>
                        <Tooltip.Content 
                          sideOffset={5} 
                          className="bg-zinc-950/95 border border-zinc-800/80 p-3 lg:p-4 rounded-xl shadow-xl flex flex-col gap-1.5 min-w-[200px] max-w-[350px] z-50 animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95"
                        >
                          <p className="text-zinc-200 font-semibold text-sm leading-tight">{formattedAction}</p>
                          <p className="text-zinc-400 text-xs leading-relaxed">{formattedDesc}</p>
                          <p className="text-zinc-500 font-mono text-[10px] mt-1 pt-2 border-t border-zinc-800/80">
                            {createdAtDate.toLocaleString()}
                          </p>
                          <Tooltip.Arrow className="fill-zinc-800/80" />
                        </Tooltip.Content>
                      </Tooltip.Portal>
                    </Tooltip.Root>
                  </div>
            ...
            {isFetchingNextPage && (
              <div className="relative flex items-center gap-4 opacity-50 p-4">
                 <div className="w-8 h-8 rounded-full border border-zinc-800/80 bg-zinc-900 shrink-0" />
                 <Skeleton className="h-6 w-full rounded-md bg-zinc-800" />
              </div>
            )}
          </div>
          </Tooltip.Provider>
